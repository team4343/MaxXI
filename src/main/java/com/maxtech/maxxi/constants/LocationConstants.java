...
public final class LocationConstants {
    ...
    public static Pose2d getClosetGrid(Pose2d robotPose) {
        /* Chose the closest grid to the robot to place. */
        if (robotPose.getTranslation().getX() <  FIELD_MIDDLE) {
            // Red side
            if (robotPose.getTranslation().getY() < 0.0) return RED_GRID_FAR.pose;
            else if (robotPose.getTranslation().getY() < 0.0) return RED_GRID_CENTER.pose;
            else return RED_GRID_PICKUP.pose;
        } else {
            // Blue side
            if (robotPose.getTranslation().getX() > FIELD_MIDDLE) return BLUE_GRID_FAR.pose;
            else if (robotPose.getTranslation().getY() < 0.0) return BLUE_GRID_CENTER.pose;
            else return BLUE_GRID_PICKUP.pose;
        }
    }
    ...
    public static final class RobotPositionConstants {
        ...
        public RobotPositionConstants plus(Transform2d transform2d) {
            this.pose.plus(transform2d);
            return this;
        }
    }
}
