...
public class ArmSubsystem extends SubsystemBase implements Loggable {
    public enum State {
        Pickup(0, 0),
        Rest(0, 0),
        PlacingA(Math.PI / 4, 0),
        PlacingB(0, 0),
        PlacingC(0, Math.PI / 2),
        PlacingD(Math.PI / 2, Math.PI / 2),
        PlacingE(0, 0);
    ...
    public ArmSubsystem() {
        ...
        // m_elbow.getEncoder().setPositionConversionFactor(1/70);
        // m_shoulder.getEncoder().setPositionConversionFactor(1/40);
        ...
        tab.addNumber("Elbow setpoint position", () -> {return m_desiredState.elbow;});
        tab.addNumber("Shoulder setpoint position", () -> {return m_desiredState.shoulder;});
    ...
    public void periodic() {
        m_elbow.getPIDController().setReference((m_desiredState.elbow / (2 * Math.PI)) * 70 * 40 / 22, ControlType.kPosition);
        m_shoulder.getPIDController().setReference((m_desiredState.shoulder / (2 * Math.PI)) * 40 * 60 / 16, ControlType.kPosition);
   }
}
